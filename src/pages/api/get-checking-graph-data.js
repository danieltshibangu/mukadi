...
import getCheckingAccess from '../../../utils/getCheckingAccess';
...
const handler = async (req, res) => {
    if (req.method === 'GET') {
        try {
            ...
            console.log(Object.values(getCheckingAccess(data)))

            const checkingTokenList = Object.values(getCheckingAccess(data))
            console.log(typeof(checkingTokenList))

            // push checking tokens into the list 
            for (let a=0; a < checkingTokenList.length; a++) {
                let user_access_code = Buffer.from(checkingTokenList[a], 'utf8').toString('base64');
